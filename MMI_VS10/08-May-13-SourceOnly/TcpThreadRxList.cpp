...
void DistributeMessage(I_MSG_RUN *pReadBuf, CTcpThreadRxList *pTcpThreadRxList)
	...
	case NET_MODE:
		...
		sprintf(s,"[%5d]NET_MODE Msg Id 0x%04x  at %s\n", 
			nPrintCount++, pReadBuf->MstrHdr.MsgId, buffer);
		TRACE(s);
#endif
	...
	default:  //not a truscan defined msg
		...
		//if ((nFlushCount & 3) == 0)
			{
#ifdef _DEBUG
			i = recv( g_nClientSocket, (char *) &Flush[0], 0x4000, 0 );
...
BOOL CTcpThreadRxList::NewSendFunction( BYTE *pBuf, int nSize,  int nDeleteFlag)
	...
	nClient = pCTscanDlg->ComputePamClientNumber();	// or put this where the command is formed and use GetPamClientNumber()
