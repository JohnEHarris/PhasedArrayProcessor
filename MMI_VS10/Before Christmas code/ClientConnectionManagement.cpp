...
How it is intended to work:
	...
	[CClientConnectionManagement]
	  ...
	  |    |--[CTCPCommunicationDialog] -- Receiver instance
	  ...
	  |    |--[CTCPCommunicationDialog] -- Sender instance
...
void CClientConnectionManagement::InitReceiveThread(void)
	...
	m_pstCCM->pReceiveThread->PostThreadMessageW(WM_USER_INIT_TCP_THREAD, (WORD) 1, (LPARAM) this);
	}
...
void CClientConnectionManagement::TimerTick(void)
	{
	if ( NULL == m_pstCCM)				return;
	if ( NULL == m_pstCCM->pReceiveThread)	return;
	m_pstCCM->pReceiveThread->PostThreadMessage(WM_USER_TIMER_TICK, (WORD) 0, (LPARAM) 0);
	}
...
// The Async socket has received a/many packet(s). Rather than handling the packet in the socket driver,
// let the next management level up store the packet into the linked list and then later process the list
// at the priority level of the CCM class or the main dlg - which should be normal priority.
// This routine is typically called by CClientSocket::OnReceive(). Hence it runs at the callers priority
// All this calling back and forth
// is done to allow different thread priorities for Sending/Receiving/Running the GUI -- if so desired
//
void CClientConnectionManagement::OnReceive(CClientSocket *pSocket)
	{
//	BYTE *pB = new BYTE[MAX_MSG_BYTES];	// Once we know what it is we will scale down the memory
	BYTE Buf[MAX_MSG_BYTES];			// put it on the stack instead of the heap. Probably quicker
	//TCPDUMMY * Data = new TCPDUMMY;
	int n;
	CString s;
	n = pSocket->Receive( (void *) Buf, MAX_MSG_BYTES, 0 );	// read all data available into Buf
	s.Format(_T("OnReceive got %d bytes"), n);
	if ( n > 0)
		{
		BYTE *pB2 = new BYTE[n];	// resize the buffer that will actually be used
		memcpy( (void *) pB2, (void *) Buf, n);	// move all data to the new buffer
		...
		// Signal the main dialog that client data has been received from this client.
...
// For the SysCp client, this routtine is not called but the ProcessReceiveMessage routine in the subclass CCM_SysCp is.
