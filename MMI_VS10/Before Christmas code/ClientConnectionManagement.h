...
#define WM_USER_TIMER_TICK							WM_USER+0x219	// also in TScanDlg.h
...
public:
	...
	// move client/server ip info up one level to CCM
	void SetClientIp(CString s);
	void SetServerIp(CString s);

#if 0
	void SetClientIp(CString s)		{ if (m_pstCCM) 
		{ if (m_pstCCM->pClientIdentity) _tcscpy_s(m_pstCCM->pClientIdentity->ClientIP, MAX_CID_IP_CHAR_LENGTH, s);} };
	 void SetServerIp(CString s)	{ if (m_pstCCM) 
...
//
PubExt1 CClientConnectionManagement *pCCM[MAX_CLIENTS];	// global, static ptrs to class instances define outside of the class definition.
