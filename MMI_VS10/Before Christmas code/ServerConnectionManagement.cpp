...
//	StopListenerThread(nMyServer);
	...
	for (i = 0; i < MAX_CLIENTS_PER_SERVER; i++)
		...
		for ( j = 0; j < 60; j++)
			...
			if (NULL != pThread)
				{
				if (m_pstSCM->pClientConnection[i]->pServerSocketOwnerThread == NULL)		// Thread has exited
#endif
...
int CServerConnectionManagement::SendPacket(int nClientIndex, BYTE *pB, int nBytes, int nDeleteFlag)
	...
	pThread->PostThreadMessage(WM_USER_SEND_TCPIP_PACKET, (WORD) nClientIndex, 0L);
	if (nDeleteFlag)		delete pB;
