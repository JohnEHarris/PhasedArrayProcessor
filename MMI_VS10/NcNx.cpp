...
void CNcNx::CopyListBoxToMessage(void)
	...
	m_pPamInstChnlInfo->bChnlRepeats = (BYTE) m_nChnlRepeat;
...
// of decades. Allocate memory the the message here to be put into a linked list.  Call the main
// dialog to actually add the message to the linked list so the message sequence number can be attached.
//
void CNcNx::SendMsg(PAM_INST_CHNL_INFO *pMsg, int nChTypes)
	{
	int nPam, nInst, nChnl;
	PAM_INST_CHNL_INFO *pSend = new PAM_INST_CHNL_INFO;
	memset ((void *) pSend,0, sizeof(PAM_INST_CHNL_INFO));

	pSend->wMsgID			= NC_NX_CMD_ID;
	pSend->bPamNumber		= m_nLastPam;
	pSend->bInstNumber		= m_nLastInst;
	pSend->bChnlTypes		= m_nChnlType;
	pSend->bChnlRepeats		= m_nChnlRepeat;
	pSend->bMaxVChnlPerInst	= 32;

	memcpy((void *)&pSend->stNcNx, (void *)pMsg->stNcNx, sizeof(ST_NC_NX)*m_nChnlType);
