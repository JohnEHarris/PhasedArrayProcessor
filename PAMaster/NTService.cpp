...
// jeh include stdafx.h 22-May-12
#include "stdafx.h"
...
#ifdef UNICODE
	Argc = (DWORD)argc;
	Argv = Argv1 = CommandLineToArgvW(GetCommandLineW(), (int *)&Argc );
#else
    Argc = (DWORD) argc;
    Argv = argv;
#endif
		...
		if( Argv[0][0] == TEXT('-') ) {
			switch( Argv[0][1] ) {
				...
				case TEXT('f'):	//!! TCW MOD faceless non-service (Win95) mode
					...
					//::GlobalFree((HGLOBAL)Argv1);
#endif

					//m_bDebug = TRUE;
					// pass original parameters to DebugService()
					m_bDebug = DebugService(argc, argv, (Argv[0][1] == TEXT('f'))); //!! TCW MOD faceless non-service (Win95) mode
#ifdef UNICODE
					::GlobalFree((HGLOBAL)Argv1);
#endif					
					return m_bDebug;// DebugService(argc, argv, (Argv[0][1] == TEXT('f'))); //!! TCW MOD faceless non-service (Win95) mode
			}
		}
	}
...
BOOL CNTService :: InstallService() {
	...
	if( OsIsWin95() ) {	//!! TCW MOD - code added to install as Win95 service
		...
		if( bRet ) {
			...
			printf("Service installed.  Type any key to exit.\r\n");
			_getch();
		}
	}	//!! TCW MOD
