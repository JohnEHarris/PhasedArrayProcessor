...
BEGIN_MESSAGE_MAP(CReplay, CDialog)
	...
	ON_MESSAGE(WM_USER_CONTINUOUS_REPLAY, OnRpPlay)
...
//					0,0);
	...
	if ( sconfig.GetLength()!=0 )
	{
		CFile file (sconfig, CFile::modeRead|CFile::shareDenyNone);	// file is on stack in OnPlay
		//m_pDataFile = (CFile *)file.Duplicate();	// persistent file
		//file.Close();
		int readSize = file.Read(&ConfigRec, sizeof(ConfigRec));
	}
...
void CReplay::OnOK() 
	...
	pCTscanDlg->SaveWindowLastPosition("REPLAY", &wp);
...
//	sconfig = ConfigRec.JobRec.CfgFile;
	...
	pCTscanDlg->GetWindowLastPosition("REPLAY", &rect);
...
//									_T("This is a test"));
	...
	if (!pIndexFile)
		{// fatal error
		MessageBox("Failed to Open Index File","Fatal Error");
		OnCancel();
		return TRUE;
		}
	...
	while (nSize == sizeof(INDEX_REC))
		...
		if (Time!=NULL)
		{
			memcpy(pCTscanDlg->m_pInspectDlg1->m_tTime , Time,sizeof(tm));
			if (m_time.GetCheck() == 1)
				s.Format("%04d-%02d-%02d  %02d:%02d   %07u      P=%d",1900+Time->tm_year,
				Time->tm_mon+1, Time->tm_mday, Time->tm_hour, 
				Time->tm_min, iRec.JointNumber,
				iRec.pos);
			else
				s.Format("%07u   %04d-%02d-%02d  %02d:%02d      P=%d", iRec.JointNumber,
